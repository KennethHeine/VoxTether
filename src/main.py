...
from .injector import TextInjector, InjectionMode
...
class VoxTetherApp:
    ...
    def __init__(self):
        ...
        self._tray_manager = TrayManager(tooltip="VoxTether")
    ...
    def _test_microphone(self) -> None:
        ...
        def test():
            result = self._recorder.test_microphone(2.0)
            ...
            if self._transcriber:
                trans_result = self._transcriber.transcribe(result.file_path)
                
                if trans_result.success and trans_result.text:
                    self._tray_manager.show_notification(
                        "Microphone Test Successful",
                        f"Heard: {trans_result.text}",
                    )
                else:
                    self._tray_manager.show_notification(
                        "Microphone Test",
                        "Recording worked, but no speech detected.",
                    )
            else:
                self._tray_manager.show_notification(
                    "Microphone Test",
                    f"Recorded {result.duration_seconds:.1f}s of audio.",
                )
